...
<article class="mod-post mod-post__type-post">
	<header>
		<h1 class="mod-post__title"><?php echo ($article["title"]); ?></h1>
	</header>
		<div class="mod-post__entry wzulli"><?php echo ($article["content"]); ?>
</div>
			<div class="mod-post__meta">
	<div>
	  <div>— 于 <time datetime=""><?php echo (date("Y-m-d  H:i",$article["create_time"])); ?></time>，<span>共写了???个字</span>；</div>
	  <div>— 文内使用到的标签：<span class="mod_tag"></span></div>
	</div>
	</div>
	<ul class="sxwk">
		<li>下一篇：
			<?php if($next_art['id'] == 0): ?><a title="没有了">&nbsp;没有了哦~~~</a><?php else: ?><a href="<?php echo U('Article/show_content',array('id' => $next_art['id'],'cate_id' => $next_art.['cate_id']));?>" rel="next"><?php echo ($next_art["title"]); ?></a><?php endif; ?>
		</li>
		<li>上一篇：
			<?php if($pre_art['id'] == 0): ?><a title="没有了">&nbsp;没有了哦~~~</a><?php else: ?><a href="<?php echo U('Article/show_content',array('id' => $pre_art['id'],'cate_id' => $pre_art.['cate_id']));?>" rel="pre"><?php echo ($pre_art["title"]); ?></a><?php endif; ?>
		</li>
	</ul>
</article>
...
<!-- You can start editing here. -->
	...
	<div id="respond" class="comment-respond">
		<h3 id="reply-title" class="comment-reply-title">发表评论 <small><a rel="nofollow" id="cancel-comment-reply-link" href="http://www.wpke.net/141.html#respond" style="display:none;">取消回复</a></small></h3>			<form action="http://www.wpke.net/wp-comments-post.php" method="post" id="commentform" class="comment-form">
...
<div id="topcontrol" title="返回顶部" style="position: fixed; bottom: 80px; right: 30px; opacity: 1; cursor: pointer;"><div class="up"></div></div></body></html>