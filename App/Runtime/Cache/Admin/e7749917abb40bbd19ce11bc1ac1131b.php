						...
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="long-td">
                                        ...
                                        <td><?php echo (date('Y年m月d日 H:i:s',$vo["add_time"])); ?></td>
                                        <td><?php echo (date('Y年m月d日 H:i:s',$vo["update_time"])); ?></td>
                                        <td>
                                        	<?php if($vo["status"] == 1): ?><span class="label label-primary">显示</span>
                                        		<?php else: ?>
                                        			<span class="label label-danger">不显示</span><?php endif; ?>
                                        </td>
                                        <td><?php echo ($vo["orderby"]); ?></td>
										<td>
											<a href="<?php echo U('Links/edit',array('id'=>$vo['id']));?>" class="btn btn-primary">
...
<script>
	...
	function del(id){
		    ...
		    layer.close(index);
		    window.location.href="/MyBlog/Admin/Links/del/id/"+id+"";
		});
	}
