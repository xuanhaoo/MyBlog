...
$(document).ready(function(){
	$("#loginout").click(function(){
						...
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="long-td">
										...
										<td><?php echo ($vo["ly_name"]); ?></td>
                                        <td style="text-align:left"><?php echo (mb_substr(strip_tags($vo["ly_content"]),0,25,'utf-8')); ?>...</td>
                                        <td><?php echo (date('Y年m月d日 H:i:s',$vo["ly_time"])); ?></td>
                                        <td><?php echo ($vo["ly_ip"]); ?></td>
                                        <td>
                                        	<?php if($vo["reply"] != null): ?><span class="label label-primary">已回复</span>
                                        		<?php else: ?>
                                        			<span class="label label-danger">未回复</span><?php endif; ?>
                                        </td>
										<td>
											<a href="<?php echo U('Liuyan/edit',array('id'=>$vo['id']));?>" class="btn btn-primary">
		...
		function del(id,p){
			...
			layer.close(index);
			window.location.href="/MyBlog/Admin/Liuyan/del/id/"+id+"/p/"+p+"";
			});
		}
		</script>
