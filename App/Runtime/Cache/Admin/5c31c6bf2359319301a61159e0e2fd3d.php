						...
						<ul class="list-group">
							<li class="list-group-item">
								<p class="text-success"><a href="<?php echo U('Index/index');?>" title="返回首页" class="tip-bottom"><i class="fa fa-home"></i> 首页</a> /
									...
									<a href="#" class="tip-bottom">编辑角色</a>
								</p>
							</li>
						</ul>
					</div>
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<form class="form-horizontal" method="post" action="<?php echo U('System/editRole');?>" name="basic_validate" id="signupForm">
							<div class="form-group">
								<label class="col-sm-2 control-label">角色名称</label>
								<div class="col-sm-6">
									<input type="hidden" name="id" id="id" value="<?php echo ($detail["id"]); ?>">
									<input type="text" id="name" name="name" placeholder="输入角色名称" value="<?php echo ((isset($detail["name"]) && ($detail["name"] !== ""))?($detail["name"]):''); ?>" class="form-control">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" name="remark" placeholder="输入备注信息" value="<?php echo ((isset($detail["remark"]) && ($detail["remark"] !== ""))?($detail["remark"]):''); ?>" class="form-control">
								</div>
							</div>
							<div class="hr-line-dashed"></div>
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									<div class="radio i-checks">
										<?php if($detail['status'] == 1 ): ?><label>
												<input type="radio" name='status' value="1" checked="checked" />启用</label>
											<label>
												<input type="radio" name='status' value="0" />停用</label><?php endif; ?>
										<?php if($detail['status'] == 0 ): ?><label>
												<input type="radio" name='status' value="1" />启用</label>
											<label>
												<input type="radio" name='status' value="0" checked="checked" />停用</label><?php endif; ?>
									</div>
								</div>
							</div>
							<div class="hr-line-dashed"></div>
...
<script>
    $(document).ready(function(){$(".i-checks").iCheck({checkboxClass:"icheckbox_square-green",radioClass:"iradio_square-green",})});
		...
		<script src="/MyBlog/Public/Admin/js/form-validate-addrole.js"></script>
	</body>
