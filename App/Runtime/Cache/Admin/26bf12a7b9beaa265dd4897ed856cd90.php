						...
						<ul class="list-group">
							<li class="list-group-item">
								<p class="text-success"><a href="<?php echo U('Index/index');?>" title="返回首页" class="tip-bottom"><i class="fa fa-home"></i> 首页</a> /
									...
									<a href="<?php echo U('System/user');?>" class="tip-bottom">用户管理</a> /
									<a href="#" class="tip-bottom">编辑用户</a>
								</p>
							</li>
						</ul>
					</div>
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<form class="form-horizontal" method="post" action="<?php echo U('System/editUser');?>" name="basic_validate" id="signupForm">
							<div class="form-group">
								<label class="col-sm-2 control-label">用户名</label>
								<div class="col-sm-6">
					<!--id>-->      <input type="hidden" name="id" id="id" value="<?php echo ($detail["id"]); ?>">
									<input type="text" name="username" id="user" value="<?php echo ((isset($detail["username"]) && ($detail["username"] !== ""))?($detail["username"]):''); ?>" class="form-control">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" id="nickname" name="nickname" value="<?php echo ((isset($detail["nickname"]) && ($detail["nickname"] !== ""))?($detail["nickname"]):''); ?>" placeholder="输入显示名称" class="form-control">
								</div>
							</div>
							<div class="hr-line-dashed"></div>
							<div class="form-group">
								...
								<div class="col-sm-6">
									<div style="width: 200px; height: 90px; float: left;">
										<input type="hidden" name="head_img" value="<?php echo ((isset($detail["head_img"]) && ($detail["head_img"] !== ""))?($detail["head_img"]):''); ?>" id="data_photo" />
										<input id="photo_file" name="photo_file" type="file" multiple="true" value="" />
									</div>
									<div style="width: 200px; height: 90px; float: left;">
										<img id="upload_img" class="img-circle" src="/MyBlog/Uploads/<?php echo ((isset($detail["head_img"]) && ($detail["head_img"] !== ""))?($detail["head_img"]):'touxiang_default.gif'); ?>" style="width: 80px;height: 80px" />
									</div>
								</div>
							</div>
							<div class="hr-line-dashed"></div>
							<div class="form-group">
				                ...
				                <div class="col-sm-6">                   
				                    <select name = "role" class="form-control m-b chosen-select">
				                      <?php if(is_array($role_list )): foreach($role_list as $key=>$var): ?><option value="<?php echo ($var["id"]); ?>" <?php if(($var["id"]) == $detail["role"]): ?>selected="selected"<?php endif; ?> ><?php echo ($var["name"]); ?></option><?php endforeach; endif; ?>                    
				                    </select>
				                </div>
				            </div>
							<div class="hr-line-dashed"></div>
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" name="remark" id="note" value="<?php echo ((isset($detail["remark"]) && ($detail["remark"] !== ""))?($detail["remark"]):''); ?>" class="form-control">
								</div>
							</div>
							<div class="hr-line-dashed"></div>
							<div class="form-group">
								<label class="col-sm-2 control-label">账号状态
								</label>
								<div class="col-sm-6">
									<div class="radio i-checks">
										<?php if($detail['status'] == 1 ): ?><input type="radio" name='status' value="1" checked="checked" />正常
					                        <input type="radio" name='status' value="0" />停用<?php endif; ?>
					                    <?php if($detail['status'] == 0 ): ?><input type="radio" name='status' value="1" />正常
					                        <input type="radio" name='status' value="0" checked="checked"  />停用<?php endif; ?>
									</div>																	
								</div>
							</div>
							<div class="hr-line-dashed"></div>
							<div class="form-group">
								<div class="col-sm-4 col-sm-offset-2">
									...
									<a class="btn btn-white" href="<?php echo U('System/user');?>">取消</a>
								</div>
							</div>
						</form>
					</div>
...
<script>
    $(document).ready(function(){$(".i-checks").iCheck({checkboxClass:"icheckbox_square-green",radioClass:"iradio_square-green",})});
		...
		<script src="/MyBlog/Public/Admin/js/plugins/validate/messages_zh.min.js"></script>
		<script src="/MyBlog/Public/Admin/js/form-validate-adduser.js"></script>
		<script type="text/javascript">
			$("#photo_file").uploadify({
				'swf': '/MyBlog/Public/Common/js/uploadify/uploadify.swf?t=<?php echo ($nowtime); ?>',
				'uploader': '<?php echo U("Upload/uploaduserimg");?>',
