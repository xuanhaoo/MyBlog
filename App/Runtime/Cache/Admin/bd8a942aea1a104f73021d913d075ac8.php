...
$(document).ready(function(){
	$("#loginout").click(function(){
		...
		<br />
		<a href="<?php echo U('Article/addMood');?>" class="btn btn-primary">New心情</a>
		<br />
		<br />
		<div class="row">
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-title">
						<h5><i class="fa fa-tasks"></i> 心情管理</h5>
					</div>
						...
						<table class="table table-bordered">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="long-td">										
										...
										<td><img class="" src="/MyBlog/Uploads/<?php echo ((isset($vo['image']) && ($vo['image'] !== ""))?($vo['image']):'userImg.jpg'); ?>" style="height: 100px" /></td>
										<td><?php echo ($vo["content"]); ?></td>				
										<td><?php echo ($vo["click"]); ?></td>
										<td><?php echo (date('Y-m-d H:i:s',$vo["create_time"])); ?></td>
										<td>
											<a href="<?php echo U('editMood',array('id'=>$vo['id']));?>" class="btn btn-primary btn-mini">
		...
		function del(id,p){
			...
			layer.close(index);
			window.location.href="/MyBlog/Admin/Article/delMood/id/"+id+"/p/"+p+"";
			});
		}
		</script>
